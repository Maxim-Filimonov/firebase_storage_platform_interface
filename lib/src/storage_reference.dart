...
class StorageReference {
  ...
  /// Returns the [FirebaseStoragePlatform] service which created this reference.
  FirebaseStoragePlatform getStorage() {
  ...
  StorageUploadTask putFile(File file, [StorageMetadata metadata]) {
    ...
    final StorageFileUploadTask task =
        StorageFileUploadTask(file, _firebaseStorage, this, metadata);
    task.start();
    return task;
  }
  ...
  StorageUploadTask putData(Uint8List data, [StorageMetadata metadata]) {
    final StorageUploadTask task =
        StorageDataUploadTask(data, _firebaseStorage, this, metadata);
    task.start();
    return task;
  }
  ...
  StorageFileDownloadTask writeToFile(File file) {
    final StorageFileDownloadTask task = StorageFileDownloadTask(
  ...
  Future<StorageMetadata> getMetadata() async {
    return StorageMetadata.fromMap(await MethodChannelFirebaseStorage.channel
  ...
  Future<StorageMetadata> updateMetadata(StorageMetadata metadata) async {
    return StorageMetadata.fromMap(await MethodChannelFirebaseStorage.channel
      ...
      'bucket': _firebaseStorage.storageBucket,
      'path': _pathComponents.join("/"),
      'metadata': metadata == null ? null : buildMetadataUploadMap(metadata),
    }));
  }
